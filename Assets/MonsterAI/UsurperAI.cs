...
public class UsurperAI : FSM
{
    ...
    public float fireCooldownTimeMain;
    ...
    {
        var projectile = (GameObject)Instantiate(fireballPrefab, fireSpawn.position, fireSpawn.rotation);
    }
    ...
    protected void UpdateStareState()
    {
        ...
        if(animator.GetBool("sightedPlayer") == false && dist <= sightRange)
        {
    ...
    protected void UpdateManicState()
    {
        ...
        float dist = Vector3.Distance(transform.position, playerTransform.position);
        int fireRoll = 50;
        int fireChance = Random.Range(0,100);
        ...
        if(dist > atkRange && fireChance >= fireRoll)
        {
            animator.SetBool("isChasing", false);
            transform.position = this.transform.position;
            ...
            else
            {
                ...
                fireCooldownTime = fireCooldownTimeMain;
            }
        }
